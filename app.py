...
# TASKS
    ...
    with st.expander("➕ Νέα Εργασία"):
        with st.form("f_task", clear_on_submit=True):
            ...
            cost = st.number_input("Συμφωνημένο κόστος (€)", min_value=0.0)
...
# ANALYTICS
    ...
    if not df_exp.empty:
        ...
        if "Χώρος" in df_exp:
            st.write("### Ανά Χώρο")
            st.bar_chart(df_exp.groupby("Χώρος")["Ποσό"].sum())
...
if menu2 == "💼 Προσφορές":
    ...
    if not df_off.empty:
        ...
        st.success(f"🏆 Καλύτερη: {best['Πάροχος']} - {best['Ποσό']}€")
        st.dataframe(df_off)
...
# CALCULATOR
    ...
    mode = st.selectbox("Επιλογή", ["Πλακάκια","Γεμίσματα","Χρώματα"])
...
# FILES
    ...
    if file:
        st.success("✔ Το αρχείο ανέβηκε (local preview)")
        st.write(file.name)
